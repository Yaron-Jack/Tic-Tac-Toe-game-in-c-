...
bool VictoryDiagonal1(vector<vector<char> > &Board, char PlayerSymbol2)///fix me
{
    for (int i = 0; i < Board.size() - 1; i++)
    {
...
bool CheckVictory(vector<vector<char> > &Board, int Player, char PlayerSymbol, char PlayerSymbol2)
{
    ...
    {
        cout << "Hooray Player " << Player << " with the symbol of " << PlayerSymbol << " has won!" << endl;
        exit(Success);
    }
}

bool
PlayersTurn(vector<vector<char> > &Board, int Player, char PlayerSymbol, vector<pair<int, int>> &UndoTurns, int Player2,
            char PlayerSymbol2)
{
...
int main()
{
    ...
    {
        cout << "\n\t Let's start! Each time a player will be selected to enter his choice of Column and Row\n\t"
        ...
        do
        {
            if (PlayersTurn(Board, 1, 'x', UndoTurns ,2, '0'))
            {
                do
                {
                    if (!(PlayersTurn(Board, 2, '0', UndoTurns,1,'x')))
                    {
                        PlayersTurn(Board, 2, '0', UndoTurns,1,'x');
                    }
                    else
                    {
                        break;
            ...
            {
                PlayersTurn(Board, 1, 'x', UndoTurns,2,'0');
            }
        } while (true);
    }
    else
...
/**************
Perhaps add a timer function - ask if to play with timer and if so to write  for how many seconds(less then 60) each play
